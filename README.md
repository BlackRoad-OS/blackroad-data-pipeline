# blackroad-data-pipeline

Part of the BlackRoad Product Suite - 100+ tools for modern development.

## Installation

```bash
curl -O https://raw.githubusercontent.com/BlackRoad-OS/blackroad-data-pipeline/main/blackroad-data-pipeline.sh
chmod +x blackroad-data-pipeline.sh
./blackroad-data-pipeline.sh
```

## About BlackRoad

BlackRoad OS is building the future of development tools and infrastructure.

🖤🛣️ **Built with BlackRoad**
